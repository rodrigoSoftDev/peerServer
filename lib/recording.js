...
module.exports = function(request, protocol, endpoint, query) {
	...
	connection.on('message', (frame) => {
		...
		if (msg.cmd==="OFFER") {
			...
			//Create recoreder
			const recorder = MediaServer.createRecorder("recordings/"+ "guid?" + query.guid + "_" + query.user + ".mp4", {	
			//	refresh : 15000,
			//	timeShift : 60000,
				disableHints : true,
			});
			...
			transport.on("targetbitrate", () => console.log("Saving video"));
